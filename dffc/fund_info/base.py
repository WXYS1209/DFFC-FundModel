...
class BaseFundInfo(object):
    ...
    def get_data_frame(self):
        ...
        df = pd.DataFrame({
            "date": date_list,
            "unite_value": [self.get_unit_value(date) for date in date_list],
            "cumulative_value": [self.get_cumulative_value(date) for date in date_list],
            "daily_growth_rate": [self.get_daily_growth_rate(date) for date in date_list],
            "purchase_state": [self.get_purchase_state(date) for date in date_list],
            "redemption_state": [self.get_redemption_state(date) for date in date_list],
            "bonus_distribution": [self.get_bonus_distribution(date) for date in date_list],
        })
        return df
