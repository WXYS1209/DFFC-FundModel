...
class StrategyExample(BackTestFuncInfo):
    ...
    def __init__(self, fund_list, start_date, end_date):
        ...
        self.threshold2 = 1.9
        self.targetposition_list = [[0.2, 0.8], [0.5, 0.5], [0.5, 0.5], [0.65, 0.35], [0.8, 0.2], [0.5, 0.5], [0.5, 0.5], [0.35, 0.65]]  # 目标仓位列表
        self.adjust_factor = 0.4  # 调整因子，控制调仓力度
    ...
    def strategy_func(self):
        ...
        if self.memory_switch == 0:
            ...
            if deltahdp > self.threshold2:
                ...
                self.memory_target_position = self.targetposition_list[4]
                self.memory_switch = 4
        elif self.memory_switch == 2:
            if deltahdp < self.threshold1:
                self.memory_target_position = self.targetposition_list[4]
                self.memory_switch = 4
            elif deltahdp < self.threshold2:
            ...
            if deltahdp < self.threshold1:
                self.memory_target_position = self.targetposition_list[4]
                self.memory_switch = 4
        elif self.memory_switch == 4:
            if deltahdp < -self.threshold2:
                self.memory_target_position = self.targetposition_list[6]
                self.memory_switch = 6
            elif deltahdp < -self.threshold1:
                self.memory_target_position = self.targetposition_list[5]
                self.memory_switch = 5
        elif self.memory_switch == 5:
            if deltahdp < -self.threshold2:
                self.memory_target_position = self.targetposition_list[6]
                self.memory_switch = 6
            elif deltahdp > -self.threshold1:
                ...
                self.memory_switch = 0
        elif self.memory_switch == 6:
            if deltahdp > -self.threshold1:
                self.memory_target_position = self.targetposition_list[0]
                self.memory_switch = 0
            elif deltahdp > -self.threshold2:
                self.memory_target_position = self.targetposition_list[7]
                self.memory_switch = 7
        elif self.memory_switch == 7:
            if deltahdp > -self.threshold1:
                self.memory_target_position = self.targetposition_list[0]
                self.memory_switch = 0
